...
function extractContentId(raw: Record<string, unknown>): string | undefined {
  const direct = toStringValue(valueOf(raw, ['content_id', 'contentId', 'コンテンツID', 'cid']));
  if (direct) return direct;
  const haystack = [
    valueOf(raw, ['url', 'URL', 'affiliate_url', 'affiliateURL', 'リンク']),
    valueOf(raw, ['product_id', 'productId', '商品ID', '品番', '品番/商品ID']),
    valueOf(raw, ['title', 'product_title', '商品名', '作品名']),
  ].map((value) => String(value ?? '')).join(' ');
  const cidParam = haystack.match(/[?&](?:cid|content_id)=([a-z0-9_-]+)/i)?.[1];
  if (cidParam) return cidParam;
  const cidLike = haystack.match(/\b([a-z]{2,6}[-_]?0*\d{2,8})\b/i)?.[1];
  return cidLike;
}
...
function normalizeRevenueRow(rawInput: unknown, source: string): RevenueReportRow {
  ...
  const contentId = extractContentId(raw);
...
export function getRevenueSignalsByPostId(): Record<string, { conversions: number; revenue: number; clicks: number }> {
  ...
  const add = (key: string | undefined, row: RevenueReportRow) => {
    if (!key) return;
    const keys = [...new Set([key, normalizeKey(key)].filter(Boolean) as string[])];
    for (const k of keys) {
      const current = signals[k] ?? { conversions: 0, revenue: 0, clicks: 0 };
  ...
  const addTitle = (title: string | undefined, row: RevenueReportRow) => {
    if (!title) return;
    const titleKey = `title:${title.trim().toLowerCase().slice(0, 80)}`;
    const current = signals[titleKey] ?? { conversions: 0, revenue: 0, clicks: 0 };
    current.conversions += row.conversions;
    current.revenue += row.revenue || row.commission || 0;
    current.clicks += row.clicks;
    signals[titleKey] = current;
  };
  for (const row of cache.rows) {
    add(row.post_id, row);
    add(row.product_id, row);
    add(row.content_id, row);
    addTitle(row.title, row);
  }
  return signals;
}
