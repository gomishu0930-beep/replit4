...
function createClient(): TwitterApi | null {
...
let _rw: ReturnType<TwitterApi['readWrite']['constructor']['prototype']['constructor']> | null = null;

function getClient(): TwitterApi {
  if (!_client) {
    _client = createClient();
    if (!_client) throw new Error('Twitter credentials not configured');
  }
  return _client;
}

function getRw() {
  if (!_rw) _rw = getClient().readWrite;
  return _rw;
}

const client = new Proxy({} as TwitterApi, { get: (_, prop) => (getClient() as any)[prop] });
const rw = new Proxy({} as ReturnType<TwitterApi['readWrite']['constructor']['prototype']['constructor']>, { get: (_, prop) => (getRw() as any)[prop] });
